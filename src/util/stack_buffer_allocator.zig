...
pub fn StackBufferAllocator(size: usize) type {
    return struct {
        buffer: [size]u8,
        end_index: usize,
        ...
        pub fn ownsPtr(self: *@This(), ptr: [*]u8) bool {
            return sliceContainsPtr(&self.buffer, ptr);
        }

        pub fn ownsSlice(self: *@This(), slice: []u8) bool {
        ...
        /// not reversible.
        pub fn isLastAllocation(self: *@This(), buf: []u8) bool {
            return buf.ptr + buf.len == @as([*]u8, @ptrCast(&self.buffer)) + self.end_index;
        }
        ...
        pub fn alloc(ctx: *anyopaque, n: usize, alignment: mem.Alignment, ra: usize) ?[*]u8 {
            ...
            const adjust_off = mem.alignPointerOffset(@as([*]u8, @ptrCast(&self.buffer)) + self.end_index, ptr_align) orelse return null;
        ...
        test StackBufferAllocator {
            var fixed_buffer_allocator = mem.validationWrap(StackBufferAllocator().init(800000 * @sizeOf(u64)));
        ...
        test reset {
            ...
            var fba = StackBufferAllocator().init(buf[0..]);
        ...
        test "reuse memory on realloc" {
            ...
            {
                var fixed_buffer_allocator = StackBufferAllocator().init(small_fixed_buffer[0..]);
                const a = fixed_buffer_allocator.allocator();
